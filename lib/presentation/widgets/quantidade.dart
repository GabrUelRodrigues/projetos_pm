...
class Quantidade extends StatefulWidget {
  ...
  const Quantidade({super.key, required this.onChangeQuantidade, required this.min, required this.max});
...
class _QuantidadeState extends State<Quantidade> {
  ...
  void incrementa(){
    ...
    if (quantidade > widget.max) {
      const snackBar = SnackBar(content: Text("Quantidade máxima atingida!"));
  ...
  void decrementa(){
    ...
    if (quantidade < widget.min) {
      const snackBar = SnackBar(content: Text("Quantidade mínima atingida!"));
      ScaffoldMessenger.of(context).showSnackBar(snackBar);
      return;
    }
